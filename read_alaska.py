...
FOLDER = USER + "\\Desktop\\ALASKA\\"

def read_xlsx(file_path): 
    global documentCount, currentPage, convSuccess
    documentCount   = 0
    currentPage = 0
    convSuccess = 0    
    ...
    for sheet in wb.worksheets:        
        if sheet.title == "LDI": # skip sheet named LDI
            pass
        else :
            for c in range(11,sheet.max_column,1):
               doc = ""
               column_letter = get_column_letter(c)
               doc = sheet[column_letter + '7'].value
               
               if str(doc).startswith("LDI"):
                    documentCount = documentCount + 1
            
            for d in range(11,sheet.max_column,1):
               si = ""
               column_letter = get_column_letter(d)
               si = sheet[column_letter + '7'].value

               if str(si).startswith("LDI"):
                    ...
                    for i in range(4,itempos, 1):            
                        newrow = str(i)
                        if sheet['A' + newrow].value is not None and isinstance(sheet['A' + newrow].value, int) and sheet[column_letter + newrow].value is not None and  isinstance(sheet[column_letter + newrow].value, int)  :
                            itemcode = sheet['A' + newrow].value
                            customer = itemcode
                            desc     = sheet['B' + newrow].value 
                            qty      = sheet[column_letter + newrow].value
                            uom      = "CS"
                    ...
                    if path.isfile(save_to_path):
                        ...
                        if documentCount == convSuccess and convSuccess != 0:
                            return 1
                        elif documentCount == convSuccess and convSuccess == 0 :
                            return 0
